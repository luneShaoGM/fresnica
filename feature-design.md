...
> Architecture argument for feature-first layout instead of a global `services/` layer. Product shell, naming, and rewrite order live in `docs/stellar-product-information-architecture.md`. Folder names in diagrams below (`history`, `portfolio`) are historical where they conflict with that guide.
