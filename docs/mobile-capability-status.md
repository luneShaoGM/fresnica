...
| History / Activity | Defined | Read-model + projections + cache repository + hydration/offline product state implemented; filter/search closure pending | Classic Horizon account operations use opaque-cursor pagination behind `StellarGateway`; refresh/load-more failures preserve accepted data, cursor cycles fail closed, and detail uses the single-operation endpoint with exact identity/account-association checks. Stable projections cover payment, create-account and ordinary issued-asset change-trust participants. Activity now hydrates the matching Realm snapshot immediately as explicitly cached/stale data, shows localized last-update/degraded state, then revalidates Horizon page 1 before enabling a fresh online cursor chain. Cache persistence preserves overlap continuity without restoring cursors, offline list/cached-detail reads survive Realm reopen, cache failures remain best-effort, and network/account changes remount by the same `networkId + classic address` partition key. Horizon remains authoritative. |
...
- History cache hydration/offline product state is implemented. The next independent S13 slice remains **current-loaded-set filter/search closure**; existing filter/search UI is not claimed as closed by this cache slice.
