...
- S05 Testnet Friendbot target `ad4cf88c...` adds a clean-room inactive-account activation path without creating a second balance or transaction authority: eligibility is fail-closed in Network Capability, the Platform adapter sends only the public `G...` address, Home de-duplicates per-account requests and reloads Balance/Horizon after success. A real host protocol smoke reached Friendbot 200 and then Horizon `10000.0000000` XLM; Android native product acceptance remains unsigned because the current AVD entered a launcher-resolver environment fault after its separate network fault was recovered. Aggregate S05 therefore remains L3 partial.
