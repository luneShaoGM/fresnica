...
| History / Activity | Defined | Read-model + projections + cache/offline + current-loaded-set filter/search implemented; explorer/product hardening pending | Classic Horizon account operations use opaque-cursor pagination behind `StellarGateway`; refresh/load-more failures preserve accepted data, cursor cycles fail closed, and detail uses the single-operation endpoint with exact identity/account-association checks. Stable projections cover payment, create-account and ordinary issued-asset change-trust participants. Activity hydrates the matching Realm snapshot as explicitly cached/stale data, revalidates Horizon page 1 before enabling a fresh online cursor chain, and keeps cache persistence cursor-free. Current-loaded-set query adds a dedicated Trustlines filter plus local search over display copy, transaction hash, operation id, participants and asset code/issuer; it never auto-pages or issues a Horizon full-history search. Query state resets with the network/account partition but survives same-account detail navigation and refresh, and manual load-more reapplies the same query. Horizon remains authoritative. |
...
- Current-loaded-set filter/search is implemented in `925cd5f76dd9a7d096c8205f31a118b7b6fd08e7`: dedicated Trustlines filtering, stable-field local search, partition-bound query reset, same-account detail/refresh retention, manual load-more reapplication, explicit no-match vs true-empty states, localized input accessibility, and screen-level regression coverage. It does not alter cache schema/cursors or query Horizon beyond the existing explicit refresh/load-more paths.
- **S13 remains `L3 partial`.** Filter/search must receive independent review and merge before explorer/product hardening begins; explorer completion and dual-platform aggregate acceptance remain open.
