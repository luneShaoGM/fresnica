...
| History / Activity | Defined | Read-model + projections + cache repository integrity implemented; hydration/filter/search pending | Classic Horizon account operations use opaque-cursor pagination behind `StellarGateway`; refresh/load-more failures preserve accepted data, cursor cycles fail closed, and detail uses the single-operation endpoint with exact identity/account-association checks. Stable projections cover payment, create-account and ordinary issued-asset change-trust participants. History now owns a Realm-independent normalized cache snapshot port, with Memory/Realm contract parity, network/account isolation, 500-entry retention, validated ready-detail persistence and partition-local corruption clearing. Activity does not consume that cache yet: stale hydration/offline UX remains the next independent slice, and Horizon remains authoritative. |
...
| Persistence | Mobile platform mechanism | Realm v5 verified with pending recovery, network default and replaceable History cache | Realm v5 preserves the existing wallet/pending/default entities and adds only `HistoryCacheSnapshotEntity`: one replaceable public-ledger snapshot per `networkId + classic account address`, containing schema version, last successful Horizon update time, canonical normalized entries and validated ready details. Persisted History data contains no cursor/accepted-cursor chain or raw Horizon payload; corrupt/incompatible History data clears only its own partition. Memory and Realm repositories share one contract, Realm close/reopen and v4→v5 migration are covered, and local account deletion clears its matching History partition through an injected port. Current slice Realm integration is 43/43 using the arm64 Node runtime required by the installed prebuild. Secrets, app passphrases, unlock keys, signer material and transaction XDR remain outside persisted History/default/pending records. |
...
- History cache repository integrity is implemented, but Activity hydration/offline product state is not. The next S13 slice must read the matching network/account Realm snapshot, present it honestly as cached/stale, revalidate Horizon page 1 before enabling load-more, preserve overlap continuity without persisted-cursor resume, and keep write failures best-effort before filter/search closure.
...
- History cache hydration/offline product state, followed by current-loaded-set search/filter closure;
