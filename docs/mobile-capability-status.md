...
| History / Activity | Defined | Read-model integrity + operation projections implemented; persistent cache/filter/search pending | Classic Horizon account operations use opaque-cursor pagination behind `StellarGateway`; refresh/load-more failures preserve accepted data, cursor cycles fail closed, and detail uses the single-operation endpoint with exact identity/account-association checks. Stable projections now cover payment, create-account and ordinary issued-asset change-trust participants. The amended S13 contract requires a network/account-scoped replaceable Realm UX cache with honest stale/offline presentation before filter/search closure; Horizon remains authoritative. |
...
- persistent History cache is not implemented yet. The amended S13 contract now requires a replaceable Realm UX snapshot keyed by network/account, stale-while-revalidate hydration, honest offline/last-updated presentation, bounded retention and no persisted-cursor resume before filter/search closure.
...
- persistent History cache/offline hydration, followed by current-loaded-set search/filter closure;
