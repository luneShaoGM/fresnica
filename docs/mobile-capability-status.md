...
| History / Activity | Defined | Read-model + projections + cache/offline + filter/search + explorer/product hardening implemented; aggregate pending | Classic Horizon account operations use opaque-cursor pagination behind `StellarGateway`; refresh/load-more failures preserve accepted data, cursor cycles fail closed, and detail uses the single-operation endpoint with exact identity/account-association checks. Stable projections cover payment, create-account and ordinary issued-asset change-trust participants. Activity hydrates the matching Realm snapshot as explicitly cached/stale data, revalidates Horizon page 1 before enabling a fresh online cursor chain, and keeps cache persistence cursor-free. Current-loaded-set query adds a dedicated Trustlines filter plus local search over stable fields without auto-paging. Operation Details now shows explicit unsupported-type/shape warnings and a Network-Capability-owned Stellar Expert projection for supported Testnet/Mainnet transaction hashes; Platform owns only the external-open side effect. External-open failure is non-blocking and localized, leaves History state intact, and remains retryable. Horizon remains authoritative. |
...
- Final local gates on the frozen code commit pass `npm run check` at **92/92 suites and 526/526 tests**, ESLint **0 errors / 21 existing warnings**, architecture/provenance/alias gates green, and locale check **3 dictionaries / 374 shared keys**.
- **S13 remains `L3 partial`.** This slice closes explorer/product hardening only. No cache schema/cursor, Payment, Trustline, Signing, Transaction or recovery semantics changed. Independent review/merge remains required before dual-platform aggregate acceptance.
