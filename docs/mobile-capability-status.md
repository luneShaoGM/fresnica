...
| --- | --- | --- | --- |
| Account | Normative | Onboarding provisioning + S02 label rename implemented | Account records, account-signer invariants, derived watch-only state, atomic account+signer registration and first-run create/import/watch-only flows. S02 account-label rename now mutates only local Account metadata through the same injected repository, trims input, no-ops unchanged labels, persists across Realm reopen, and refreshes App bootstrap after save. Sort/hide/delete remain incomplete. |
