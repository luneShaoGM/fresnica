...
| S02 | Account list/select/label/sort/hide/delete | accounts + Account | Adapt | clean-room | persistence lifecycle | L2/L3 partial |
...
- This evidence closes the **selection/picker sub-slice only**. It does not itself promote aggregate S02 beyond `L2/L3 partial`: a current-main Android+iOS aggregate `list → direct select → restart/default restore → rename → reorder → hide/restore → delete → restart` product flow plus remaining localization/accessibility acceptance is still required before the aggregate maturity decision is revisited.
