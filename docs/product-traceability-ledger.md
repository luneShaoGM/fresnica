...
- Stacked lifecycle branch `stage4/s02-account-lifecycle-2026-09-11` keeps PR #39 rename-only while adding narrow local visibility/delete policy. The last visible account cannot be hidden; hidden records remain in Settings → Accounts and can be restored, while Home/account selection filters them out.
...
- Selected-account reconciliation now uses stable `sortOrder → createdAt → id` ordering and chooses the next previously visible account after the current account is hidden or removed. Delete UI uses explicit second confirmation and states that only the local wallet record is removed, not the Stellar on-chain account. Navigation/bootstrap refresh happens only after repository deletion succeeds; failure remains on Account Detail.
- Local validation at `26b8c6e`: `npm run check` **60/60 suites, 309/309 tests**, ESLint **0 errors / 23 warnings**, architecture/alias/provenance/locale gates passed; locale dictionaries share **122 keys**; donor gate scanned **359 files with 0 unapproved exact donor blobs**. `npm run test:realm` passed **23/23** and includes hidden close/reopen persistence plus final-account deletion persistence. Simulator/native product validation for hide/delete remains outstanding, and sort/reorder remains pending, so S02 stays `L2/L3 partial`.
