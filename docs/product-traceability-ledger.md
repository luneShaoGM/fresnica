...
- This evidence closes **rename only**. Hide/show and local delete are closed by the separate lifecycle evidence below; S02 remains `L2/L3 partial` because sort/reorder and other remaining account-lifecycle acceptance are still pending.
...
- Android emulator native acceptance on 2026-09-11 proved hide/restore persistence across restart, last-visible Hide/Delete guards, unresolved pending deletion blocking, stable selected-account takeover, shared-signer retention until reference count reaches zero, final-account deletion into the existing Onboarding bootstrap, and a real Realm write-failure path that stays on Account Detail without a success refresh/navigation. This closes the **Hide/Delete sub-slice only**; sort/reorder remains pending, so S02 stays `L2/L3 partial`.
