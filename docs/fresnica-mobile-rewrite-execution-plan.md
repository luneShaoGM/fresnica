...
- 原生安全与持久化：审计时 Mobile 使用 Fresnica Native SDK 0.2.1；Stage 1 先升级到发布的 0.3.0，2026-09-16 maintenance 再将消费基线推进到 0.3.1。Native Binding API 3 / SDK API 5 / Core Client API 5 均未变化；0.3.1 的 native-only passphrase verification 不在 canonical RN adapter JavaScript contract 内，因此 existing-wallet protected-signer 产品入口仍 fail closed。
...
- 0.3.1 新增 native-only `FresnicaSignerAuthorization.verifyProtectedSignerPassphrase(...)`，会校验 protected envelope + app passphrase + expected signer identity，并在返回前擦除派生的 `WalletUnlockKey`。当前 canonical RN adapter 未导出该方法，因此本 maintenance **不**改变 Add Account 产品语义；existing-wallet protected signer 继续 fail closed。
...
- Settings 与 Home 的 Add Account 复用同一 watch-only Screen，但各自保留自己的 native-stack 返回历史。现有钱包新增 protected software signer 继续 fail closed：Fresnica Native SDK 0.3.1 已新增 native-only `verifyProtectedSignerPassphrase`，但 canonical React Native adapter source 0.3.0 尚未把该方法导出到 JavaScript；Mobile 不建立私有旁路 bridge。
