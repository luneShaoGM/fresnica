...
- S02 Hide/Show + local Delete 已在堆叠分支 `stage4/s02-account-lifecycle-2026-09-11` 落地首轮实现：最后一个可见账户禁止隐藏；隐藏仅从 Home/账户切换选择集合移除，Settings → Accounts 继续显示全部本机账户并允许恢复。Delete 使用独立 Account capability policy，在 Realm 删除前阻断该账户任何 unresolved `submitting/uncertain` submission，并复用既有原子 account/reference/orphan-signer 删除语义；shared signer 仅在最终引用移除后清理。真正最后一个账户允许删除，删除后由既有 `resolveOnboardingBootstrap` 判定 `onboarding`；若仍有隐藏账户，则禁止删除最后一个可见账户，避免产生“有账户但无可见账户”的第三启动状态。当前账户被隐藏/删除后按 `sortOrder → createdAt → id` 稳定顺序选择下一个可见账户。Account Detail 增加本机删除二次确认，并明确不影响 Stellar 链上账户；Realm 成功前不切路由，失败保持当前页面。HEAD `26b8c6e` 本地验证：`npm run check` 60/60 suites、309/309 tests，0 errors / 23 warnings；`npm run test:realm` 23/23；locale 3 份字典统一 122 keys；provenance 359 files / 0 unapproved exact donor blobs。该证据完成实现与自动化门，但尚缺 simulator/native 手工产品验收，因此 S02 仍为 partial；sort/reorder 也仍未完成。
