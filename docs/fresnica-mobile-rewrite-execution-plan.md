...
- 原生安全与持久化：审计时 Mobile 使用 Fresnica Native SDK 0.2.1；Stage 1 先升级到发布的 0.3.0，2026-09-16 maintenance 再将消费基线推进到 0.3.1。Native Binding API 3 / SDK API 5 / Core Client API 5 均未变化；随后 canonical RN adapter `c5eae08...` 已将 `verifyProtectedSignerPassphrase` 暴露给 JavaScript，Mobile 的安全当前密码验证机制 blocker 已解除。Existing-wallet Create/Import 产品入口仍未实现，继续 fail closed，等待独立行为规格和 S01 产品切片。
...
#### Stage 1 maintenance — Native SDK 0.3.1（2026-09-16）

- 上游发布 `native-sdk-v0.3.1@984a741ab49ed5ca3eeab6da525bcacac5dd5d04`；Native Binding API 3 / Universal SDK API 5 / Core Client API 5 均保持不变，Android minSdk 26 与 Apple iOS 13.4 最低版本也不变。
- Mobile 只推进已发布 native binary、compatibility manifest、pod/AAR 引用和 dedicated native gates；canonical RN adapter source package 仍是 0.3.0，并固定到同一 0.3.1 release commit，使其 `adapter-contract.json` 明确要求 Native SDK 0.3.1。
- 0.3.1 新增 native-only `FresnicaSignerAuthorization.verifyProtectedSignerPassphrase(...)`，会校验 protected envelope + app passphrase + expected signer identity，并在返回前擦除派生的 `WalletUnlockKey`。该 SDK maintenance 的 reviewed adapter pin `984a741...` 当时尚未导出此方法；后续 S01 mechanism slice 改由 canonical adapter `c5eae08...` 消除这一历史 blocker。
- Android/Apple release artifacts 必须继续按 release `SHA256SUMS` 与固定 digest 双重验证；现有 #128/#129 checkout-only compatibility patches 仅在 adapter build 阶段保留，不进入产品签名语义。
- 精确实现提交 `e6f4098bdcfcf1700fc1546120dfce3ff97214bf` 完成 0.3.1 binary/compatibility/pod/AAR/gate pin；后续 `66b41c67ffe6efdb4b3771efd208e5f425ab5976` 仅把 Android runtime-smoke launcher 从随机 `monkey` 改成确定性的 `.MainActivity` 启动，不改变产品或 Native bridge 语义。上游 0.3.1 `adapter-manifest.mjs` 已对本地生成物重新验真：RN 0.87.0 / Native 0.3.1 / Binding API 3 / adapter source 0.3.0，Android adapter SHA-256 `0f32437e...`、Apple adapter SHA-256 `7dc5ebac...` 与 manifest 一致；release AAR/Apple package 继续由官方 `SHA256SUMS` 固定为 `2acbeb06...` / `97545f6b...`。
- 本地 `npm run check` 为 **68/68 suites、353/353 tests**、ESLint **0 errors / 23 warnings**、provenance **382 files / 0 unapproved exact donor blobs**、locale **138 keys**；匹配 Realm arm64 prebuild 的 Node 下 `npm run test:realm` 为 **2/2 suites、28/28 tests**。iOS iPhone 15 Pro simulator 完成 0.3.1 fresh Xcode rebuild 并在 fresh Metro 上返回 `FRESNICA_PARSE_ACCOUNT_SMOKE_OK`；Android 临时全新 Android 16 / API 36 AVD 使用标准 `npm run smoke:android -- --rebuild` 完成 Gradle build/install/launch 并返回同一 marker，callback 同时证明 Realm round-trip、Classic parse、`invalid-input`、external-signing bridge 与 SEP-53 bridge。该 maintenance 因此完成本地双端 runtime 验证，但不改变 Stage 4 产品成熟度；远端四项门只在 PR 精确 HEAD 上另行登记。

#### Stage 4 S01 prerequisite — canonical protected-signer passphrase verification（2026-09-16）

- Base `main@2d44dcac97472da83b7b8b0e5ea94383d983c241`；机制代码目标 `32c16852d9d57575234ece9119d20297e243e654`。Native SDK 保持 0.3.1，不升级 Core/SDK API；仅将 canonical RN adapter revision 从 `984a741...` 推进到 `c5eae08e84b197d534a05f02ae1a230a1e245f28`，adapter source package 仍为 0.3.0。
...
- Import 行为规格冻结在 `2867dce96d22c2cddf597346913f660c799d1e2a`；Repository 原子 watch-only upgrade 在 `c49b576a44f3a6ad1a70ad0cf7b6061652bab215`，Import Capability/共享保护编排在 `260b10717e84be1aa59e0aa388bee303ce4ed1f7`，Add Account UI/navigation/localization 在 `2824313894c8bd97d57db0343459c8c086d7c44e`，最终代码/native-smoke 目标为 `e8405af9a1c500787f45a2c99599934deba35b20`。
- Existing-wallet Add Account 现开放 `Import recovery phrase` 与 `Import Stellar secret`。已有 protected signer 时仍复用 Create 的 `authorizeExistingWalletProtection`，先验证所有唯一 protected target；没有 protected signer 时复用同一首个 App Passphrase 强度/确认策略。System Auth 同样复用 post-persistence helper，不建立第二套 orchestration。
- Import mnemonic 保留传给 canonical `protectMnemonic` 的 language、mnemonic passphrase 与 derivation index；secret 只交给 `protectSecret`。两类导入 signer 均持久化为 `backupState=not-required`，不创建 pending-backup。恢复材料、App Passphrase 与 envelope 不进入 navigation params、日志、默认账户 metadata 或用户错误文案。
- 同 network 的 visible watch-only 账户通过 Repository 单事务只新增 Signer + reference，原 Account id/label/order/visibility 等元数据保持不变；同 network 已有签名权限稳定返回 `account-already-exists`；hidden watch-only 不被 Import 隐式 restore，而返回 `account-not-selectable`；不同 network 的同公钥继续是独立 Account identity。成功导入后沿用既有 `selectPersistedAccountAndDefault`，default 写成功后才改变 current；default 写失败时旧 current/default 保持权威，已持久化的 Import 可留待显式重选。
- 精确代码 HEAD `e8405af...` 的 `npm run check` 为 **74/74 suites、397/397 tests**、ESLint **0 errors / 21 warnings**、provenance **394 files / 0 unapproved exact donor blobs**、locale **191 keys**。同一产品树的 Realm 集成为 **2/2 suites、31/31 tests**；之后只对 native-smoke fixture 做确定性 seed amend，未修改 Realm/产品代码。
...
- session lock 的纯状态机已覆盖 cold-start、background timeout、foreground resume、cancel 与 verified-unlock transition；但 Fresnica Native SDK 0.3.1 所配套的 canonical React Native adapter source 0.3.0 仍没有通用 `authenticateSystemAuth(reason)`。在上游提供安全 challenge 前，不得用 reveal、dummy XDR、伪 biometric probe 或 JavaScript credential scheme 绕过。
...
5. Activity
   ...
   - 按 `networkId + account address` 隔离持久化公开账本 UX snapshot；进入页面先恢复缓存、后台由 Horizon revalidate，离线时明确显示缓存状态与最后更新时间。
   - 缓存恢复态不续跑持久化 cursor；成功刷新 page 1 后重建在线分页链。缓存不是完整历史、gap recovery 或账本真值。
...
当前执行证据（截至 2026-09-15）：

- S02 Account label rename 已在 2026-09-11 闭合为独立子切片：Account Capability 只更新本地 `label + updatedAt`，输入 trim，trim 后未变化则不写；Memory/Realm 共用同一窄 Repository contract，Realm close/reopen 保留名称。Settings → Accounts → Account Detail 使用共享 `AppModal + Field + Button` 编辑，保存失败仅显示稳定本地化错误，成功后经 `onAccountsChanged()` 重新解析同一账户真值。Android emulator 实测 `BeforeRename → BeforeRenameAfter` 在 Detail、Accounts 列表和 force-stop/restart 后 Home 均一致。此证据只完成 rename；Hide/Delete 与 Sort/reorder 分别由后续独立证据闭合。S02 的整体成熟度仍按聚合验收单独评定。
- S02 Hide/Show + local Delete 已在独立分支 `stage4/s02-account-lifecycle-2026-09-11` 完成实现与验收：最后一个可见账户禁止隐藏；隐藏仅从 Home/账户切换选择集合移除，Settings → Accounts 继续显示全部本机账户并允许恢复。Delete 使用独立 Account capability policy，在 Realm 删除前阻断该账户任何 unresolved `submitting/uncertain` submission，并复用既有原子 account/reference/orphan-signer 删除语义；shared signer 仅在最终引用移除后清理。真正最后一个账户允许删除，删除后由既有 `resolveOnboardingBootstrap` 判定 `onboarding`；若仍有隐藏账户，则禁止删除最后一个可见账户，避免产生“有账户但无可见账户”的第三启动状态。当前账户被隐藏/删除后按 `sortOrder → createdAt → id` 稳定顺序选择下一个可见账户。Account Detail 增加本机删除二次确认，并明确不影响 Stellar 链上账户；Realm 成功前不切路由，失败保持当前页面。生命周期实现与 native 验收目标为 `a75914b25efb6959ca22b0e7582c96e21e8e5140`：`npm run check` 60/60 suites、309/309 tests，0 errors / 23 warnings；`npm run test:realm` 23/23；locale 3 份字典统一 122 keys；provenance 359 files / 0 unapproved exact donor blobs。Android emulator 人工验收覆盖 hide/restore/restart 持久化、最后可见账户 Hide/Delete guard、unresolved pending delete guard、当前账户切换的稳定顺序、shared signer 引用计数清理、删除最后账户进入既有 Onboarding bootstrap，以及真实 Realm 写失败时不导航且不触发虚假 refresh。该 Hide/Delete 子切片已闭合；Sort/reorder 由下一条独立证据验收，不反向修改这里冻结的生命周期策略。
- S02 Sort/reorder 已在独立分支 `stage4/s02-account-sort-2026-09-11` 完成功能与 Android native 首轮验收，代码/native 目标 `fc169947ddcf76634c7846f517afd1b003c18459`。排序只写本地 Account `sortOrder + updatedAt`，批量 Realm 写入原子化，稳定顺序统一为 `sortOrder → createdAt → id`；新账户追加到现有最大 `sortOrder` 之后。Settings → Accounts 提供中性 Move up/Move down 控件，hidden 账户仍留在同一全局本地顺序中，移动不会改变 hidden 状态；Home 继续只过滤 visible 账户。运行中重排当前账户不会改变 selected account identity。Android emulator 从 `11, SortB, SortC` 将当前账户 `11` 下移后得到 `SortB, 11, SortC`，Home 仍显示 `Current account 11`；随后将 hidden `SortC` 上移得到 `SortB, SortC, 11`，其 `Hidden` 状态保持，并在 force-stop/restart 后恢复相同列表顺序与 hidden 状态。cold start 仍按既有 transient selected-account 语义选择首个 visible account，本切片没有新增 selected-account 持久化。2026-09-14 review 收尾先由 `fc477902e5d41600d6aa2dc68a28054d46ac1620` 增加 disabled 视觉态与 `accessibilityState.disabled`、失败主动播报和独立 `styles.ts`；最终 review-fix `dcca4923ac978631f2d5101dbc8dee7b05105201` 保留 `AccessibilityInfo.announceForAccessibility` 作为唯一主动播报路径，移除同一错误文本的 assertive live region 以避免 TalkBack 重复播报，并用 Prettier 格式化 `AccountsScreen.tsx`、`styles.ts`、`AccountsScreen.test.ts`。最小 UI 测试新增重复播报回归覆盖。最终代码 HEAD 本地 `npm run check` 为 62/62 suites、321/321 tests，0 errors / 23 warnings；`npm run test:realm` 为 2 suites、26/26 tests；独立 `npx prettier --check` 对上述三个文件通过；locale 3 份字典统一 125 keys；provenance 363 files / 0 unapproved exact donor blobs。排序 capability、Realm、账户选择以及冻结的 Rename/Hide/Delete 逻辑均未改动。该证据关闭 Sort/reorder 主要子切片，但 S02 整体仍保持 `L2/L3 partial`，等待独立聚合成熟度评审。
- **S02 aggregate maturity review（2026-09-14，`main@4e9101b13cbf26e96e7454192fd34884150c86d9`）结论：继续保持 `L2/L3 partial`。** Rename、Hide/Delete、Sort/reorder 三个子切片的 Capability/Realm/导航组合关系复核无新阻断：rename 不改排序/可见性/Signer；hidden 账户保留全局顺序且 Home 只消费 visible；当前账户仍 visible 时排序不会切换 identity；当前账户被 hide/delete 后按既有稳定顺序接管；Delete 继续受 unresolved submission 与最后可见账户 guard 约束。当前 main 定向 S02 为 7/7 suites、32/32 tests，完整 `npm run check` 为 62/62 suites、321/321 tests，provenance 363/0，Realm 在匹配 arm64 host 的 Node 下为 26/26；merge tree 与已通过 CI #365、Realm #168、Native Android #220、Native Apple #215 的 `22a61192...` tree 完全一致。**不升级 L3 的主要原因是 S02 的 `select` 子行为尚未签收**：参考行为提供可直接选择任意 visible account 的 account picker/switcher 并保存 default account，而当前 target 仅在 Home 点击后通过 `nextVisibleAccountId()` 循环到下一个账户，`selectedAccountId` 仍是 session-local state；`docs/stellar-source-parity.md` 也仍将 account picker 标为 `BEHAVIOR_MISMATCH`。除非产品明确接受这种差异为最终 Adapt，否则必须先完成直接选择 UX 与 selected/default-account persistence policy。**L4 还额外缺少当前 main 的聚合账户产品流和 iOS S02 产品流证据**：现有 product-flow harness 仅支持 Android 且覆盖 onboarding/balance/send，不覆盖 S02；Android 账户证据仍分散在子切片历史 heads，Apple gate 只证明 build + FresnicaCore/Realm runtime smoke，并非账户 UI/E2E；CI 也尚未把 S02 product flow 作为必过门。最后，Accounts/Account Detail 仍存在硬编码英文 copy，rename/hide/delete 的 accessibility/dynamic-font/screen-reader-order 也缺少与 sort 同级的完整 UI 验收，这些属于 §7 Stage 4 / §9 的 L4 完成门。下一最小产品任务应是独立 **S02 selection/picker closure**，随后再做一条覆盖 list/select/rename/sort/hide/restore/delete/restart 的 Android+iOS aggregate product-flow acceptance；全局 `format:check` 覆盖 `src/**` 继续作为独立工程质量 PR，不混入 S02 产品逻辑。
- S02 direct selection/default persistence 已在独立分支 `stage4/s02-account-selection-2026-09-14` 完成功能与 Android native 验收，代码/native 目标 `fa315e0d8a8eefe68347de6365146557f4b0bd60`。Home 账户入口不再循环到“下一个”账户，而是打开直接选择 picker；picker 只列当前 network 的 visible accounts，并复用 `sortOrder → createdAt → id`。`selectedAccountId` 仍由 `app/` 持有；Realm v4 新增按 `networkId` 隔离的 `DefaultAccountPreferenceEntity`，不污染 Account record。显式选择先持久化 default，成功后才更新 session；写失败保持当前 session 与 picker，不宣称成功。missing/hidden/deleted/cross-network default 会归一化到稳定 visible fallback；当前默认账户被 hide/delete 后由既有 selection reconciliation 接管并回写 default，不改冻结的 Hide/Delete Capability；restore hidden account 不抢回 default；真正最后账户删除进入既有 Onboarding 时清除该 network default。v2→v4 与当前 v3→v4 Realm migration、per-network isolation、close/reopen、same-network overwrite 和 clear 均有集成覆盖。精确实现 HEAD 的干净 worktree 验证为 `npm run check` **64/64 suites、339/339 tests**、0 errors / 23 warnings，`npm run test:realm` **28/28**，locale **129 keys**，provenance **370 files / 0 unapproved exact donor blobs**，targeted source Prettier 与 `git diff --check` 通过；远端 CI #368、Realm Integration #171、Native Android Gate #223、Native Apple Gate #218 全部成功。Android emulator `emulator-5554`（`sdk_gphone64_arm64` / API 36）实测 `11 → PickB` 直接选择，force-stop/restart 仍为 `PickB`；隐藏当前 `PickB` 后 Home 稳定 fallback 到 `11` 且 restart 保持 `11`；恢复 `PickB` 后再次 restart 仍为 `11`，同时两个账户均 visible，证明 restore 不会抢回 default。selection 子切片因此具备独立复审所需证据，但 S02 聚合成熟度仍保持 `L2/L3 partial`；Android+iOS 聚合产品流与完整 localization/accessibility 仍由后续 aggregate review 决定。 2026-09-15 独立复审发现的 zero-visible-current-network P2 已由 `09b0e8bc236d878685432c197564c43248529e5b` 收口：preferred resolver 在当前 network 无 visible account 时返回空选择而不抛错，MainTabs 改走仅包含当前 network records 的 Settings/Accounts recovery stack，使 hidden 账户可 Restore、无当前 network account 时可 Add，并禁止把其他 network visible account 当作 fallback。PR #43 同时把已批准 PR #42 aggregate review commit `3274872e1b3f77457ecc4d8c313fd673f5735270` 纳入父历史，固定合并顺序为 #42 → #43；窄复核随后通过，#42 先合并，#43 同步新 `main` 后在精确 HEAD 重新过门并合并为 `48b77acc580b8dd47ea7f68b581c5c49ca67d8fb`。该 selection 子切片进入 main 时仍保持 `L2/L3 partial`，其后的双平台 aggregate acceptance 由下一条单独升级成熟度。
- **S02 dual-platform aggregate acceptance（2026-09-15，`main@48b77acc580b8dd47ea7f68b581c5c49ca67d8fb`）已把整体成熟度从 `L2/L3 partial` 升为 `L3 / L4 partial`。** Android `emulator-5554`（Android 16 / API 36）和 iOS iPhone 15 Pro simulator（iOS 17.2 / Xcode 16.2）分别完成 `list → direct select → restart/default restore → rename → reorder → hide/restore → delete → restart` 的生产接线验收；显式选择与 network-scoped default 一致，hide 当前 default 后 stable fallback 被持久化，restore 不抢 default，reorder 不改变当前账户，最终账户删除清除 default 并经既有 bootstrap 进入 Onboarding。当前 network 全 hidden 以及完全零账户时，双端都只进入该 network 的 Accounts recovery，支持 Restore/Add，且不会采用其他 network 的 visible account。Android system back、iOS edge swipe/back、Modal 关闭均实测；当前-main guard suites 为 3/3、27/27，Realm 为 28/28。L4 暂不签收：中文 locale + Android 1.5x 字体 / iOS `accessibility-large` 虽可用，但 Accounts/Account Detail 仍存在硬编码英文；TalkBack/VoiceOver 已真实启用并执行 traversal，但自动 focus-order 证据不足以签收屏幕阅读顺序；CI 也尚未把 S02 aggregate UI flow 设为必过门。验收后双端测试数据、读屏开关与系统字号已恢复。
- **S05 Testnet Friendbot 子切片（2026-09-16，代码目标 `373912dc0a1753692436b747b7b52c5493d48cc5`）已完成 L3 生产接线。** 独立 clean-room 规格固定 Testnet-only、Classic-account-only、public-address-only 与 Horizon-authoritative 约束；Network Capability 负责资格判定，`StellarFriendbotGateway` 只负责 Friendbot HTTP 机制，Home 只在 inactive Testnet Classic account 暴露入口。请求进行中按账户阻止重复触发；成功后不写本地余额而是重新走现有 Balance/Horizon 读取；失败保留 inactive 状态、显示本地化可重试错误；切换账户后旧请求不会刷新新账户。真实 macOS Node 协议 smoke 已证明 Friendbot 200 后 Horizon 可读取同一随机 Testnet 地址并返回 `10000.0000000` XLM；本地 `npm run check` 为 **68/68 suites、353/353 tests**、ESLint **0 errors / 23 warnings**，provenance **382 files / 0 unapproved exact donor blobs**，locale **138 keys**；匹配 arm64 Realm prebuild 的 Node 22.22.0 下 `npm run test:realm` 为 **2/2 suites、28/28 tests**。既有 `Medium_Phone` / `emulator-5554` 在验收时暴露 AVD 环境故障：冷启动网络无路由可通过 data toggle 恢复，但该实例随后出现 Package Manager 可见 `MainActivity` 而 launcher resolver 无法解析的损坏状态；此旧实例故障不计产品失败。为隔离环境变量，另建临时全新 `Fresnica_S05_Acceptance`（Android 16 / API 36，`emulator-5556`）执行生产 UI：fresh watch-only Testnet 账户先由 Horizon 判定 inactive，Home 真实显示 Friendbot/Refresh；点击后按钮立即变为 disabled 的 `Funding with Friendbot…`，随后 inactive 面板消失并经既有 Balance/Horizon 路径显示 `XLM 10000.0000000`，宿主 Horizon 对同一地址返回相同余额；force-stop/relaunch 后仍由持久化账户重新加载该链上余额。第二个 inactive watch-only 账户在 Friendbot in-flight 时立即切回第一账户，第二地址最终也在 Horizon 成为 `10000.0000000`，但当前 Home 始终保持第一账户，证明 stale completion 未覆盖新选择；随后直接选择第二账户亦显示其权威 XLM 余额。**Friendbot Android native 子切片因此签收 L3**；S05 聚合状态继续保持 `L3 partial`，L4 仍需 iOS 产品流、标准必过门与 Home/Balance 剩余聚合闭环。
- **S05 Home/Balance dual-platform aggregate acceptance（2026-09-17，`main@ab794fa78829a66c403a9c7f00e131d35e350ca3`）已把整体成熟度从 `L3 partial` 升为 `L3 / L4 partial`。** 无产品代码变更。Android 一次性 API 36 AVD 覆盖 active protected / funded read-only / inactive、`asset-details` 权威重读、Asset Details 返回 Home 的 focus invalidation、断网错误态、网络恢复后 `Try again`、以及 pull-to-refresh；active/read-only 均由 Horizon 返回 `XLM 10000.0000000`，inactive 不创建本地余额，Friendbot UI 只观察不重复触发。iOS 一次性 iPhone 15 Pro / iOS 17.2 使用同一 main 的 fresh signed Debug build，生产 Home/Asset Details 验证 active `10000.0000000`，生产 Home 另外验证 read-only `10000.0000000` + signing disabled 与 inactive panel/Friendbot visible。iOS 错误恢复通过仅存在于验收入口的临时 XHR Horizon fault injector 进入 `Balances unavailable / Try again`，关闭 fault 后一次 retry 回到同一权威余额；错误注入和 default-select fixture 均在验收后删除，`index.js` 恢复 main，临时双端设备与 Metro 清理完成。iOS 没有重复 Android 已证明的 focus-return fault，因此不把该平台深度写成完整 L4 E2E。**L4 仍缺 S05 aggregate 作为标准必过 CI/E2E gate，以及可信的动态字体和 TalkBack/VoiceOver focus-order 验收。**
...
- Settings 与 Home 的 Add Account 当前仍复用同一 watch-only Screen，并各自保留自己的 native-stack 返回历史。Canonical RN adapter `c5eae08...` 已安全暴露 `verifyProtectedSignerPassphrase`，因此“无法验证现有 App Passphrase”的上游技术 blocker 已解除；但 Existing-wallet Create/Import 的行为规格、全 signer 验证策略、原子持久化/default 切换与 System Auth repair 尚未实现，所以产品入口继续 fail closed，不能据此提升 S01 成熟度。
...
6. 上述 S07 iOS cancel 缺口只阻止 S07 成熟度升级和最终发布签收，不再阻塞已完成 aggregate acceptance 的 S30 Trustline；S13 Activity 仍独立保持 `L3 partial`。S13 read-model integrity、History cache contract、repository integrity、hydration/offline、current-loaded-set filter/search 与 explorer/product-hardening 均已独立闭合，PR #69 又修复了 aggregate 首轮发现的 RN URL query 丢失缺陷，并以 reviewed head `cc1578f9...` 合并为 `main@bb7a24d...`。2026-09-22 post-#69 aggregate 已确认 Android 的完整产品链，以及 iOS 的 page 1/pagination、filter/search、details/participants、cache/offline 与 production History/Realm reconnect mechanics；但 iOS 最终 reconnect 证据来自直接 helper 路径，不能表述为 Activity effect/Retry 产品流闭合。后续窄补证据又发现当前 iOS Hermes runtime 的 `Intl.NumberFormat.prototype.formatToParts` 为 `undefined`，真实 Activity row 在 `src/locale/localization.ts:117` 进入 Render Error，因此成熟度不升级。下一步固定为 **独立 iOS localization/runtime compatibility 修复 → 仅重跑缺失的 offline cached Activity → Retry → page-1 revalidation → cold restart → 再评估 S13 成熟度 → 其他 Stage 4 功能 → Stage 9 / release candidate 前真实 iPhone S07 cancel acceptance**。不得为补证据修改 History/Transaction 语义。
7. UI 最终视觉稿仍不阻塞上述功能施工；但新触及文案必须进入 locale，Accessibility label、动态字体和读屏顺序按切片记录，避免把可预见的 L4 债务继续扩大。
8. Activity/Operation Detail 的公开账本 UX cache 已按 §4.2 与 S13 behavior spec 分层闭合：repository integrity 在 `main@74229cd...`，cache hydration/offline product state 在 PR #66 / `main@02e309b8...`，current-loaded-set filter/search 在 PR #67 / `main@9aaef8c...`，explorer/product hardening 在 PR #68 后闭合；PR #69 / `main@bb7a24d...` 进一步修正 RN URL compat 对 Horizon `order/limit` query 的保留。当前 aggregate 不得宣称双端产品闭合：Android 产品链成立，iOS 的 direct History/Realm reconnect 只保留为 capability 诊断证据，真实 Activity Retry reconnect 尚缺。S13 因此继续 `L3 partial`；先独立修复 iOS Activity 所需的 NumberFormat/runtime compatibility，再只重跑缺失的 iOS offline Retry/revalidation/cold-restart 路径。其他缓存 schema/失效、Backend 边界、Mainnet/Developer Mode、最终 Application ID/iOS identity 与 release signing 继续按 Stage 8/8B/9 顺序推进，不提前宣称 L4 或发布成熟。
