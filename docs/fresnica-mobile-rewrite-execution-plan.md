...
6. 上述 S07 iOS cancel 缺口只阻止 S07 成熟度升级和最终发布签收，不再阻塞已完成 aggregate acceptance 的 S30 Trustline 或 S13 Activity。S30 已完成窄产品 hardening、独立 review/merge 与 post-#59 双端 aggregate evidence；S13 read-model integrity、History cache contract、repository integrity、hydration/offline、current-loaded-set filter/search 与 explorer/product-hardening 均已独立闭合，PR #69 又修复了 aggregate 首轮发现的 RN URL query 丢失缺陷，并以 reviewed head `cc1578f9...` 合并为 `main@bb7a24d...`。2026-09-22 fresh Android+iOS aggregate 已从 page 1 重新执行并闭合 **online page 1 → pagination → filter/search → operation details/participants → explorer failure/retry → restart/cache hydration → offline list/detail → reconnect/page-1 revalidation**，确认 network/account 分区隔离、last-updated 保留/前移及 persistent cursor 不恢复。S13 因此提升为 **`L3 / L4 partial`**；剩余仅为 repeatable required aggregate gate、动态字体和可信 TalkBack/VoiceOver focus-order/tap-driven L4 深度，不得为补证据反向增加 History/Transaction 产品语义。后续主线顺序继续进入 **其他 Stage 4 功能 → Stage 9 / release candidate 前真实 iPhone S07 cancel acceptance**。release candidate 前必须实机闭合 `cancel → user-cancel → pending=0 → retry → submitted`；S13 与后续写账切片继续复用 Stage 2.5 transaction recovery，不因该证据延期降低 exact-XDR / fail-closed 要求。
7. UI 最终视觉稿仍不阻塞上述功能施工；但新触及文案必须进入 locale，Accessibility label、动态字体和读屏顺序按切片记录，避免把可预见的 L4 债务继续扩大。
8. Activity/Operation Detail 的公开账本 UX cache 已按 §4.2 与 S13 behavior spec 分层闭合：repository integrity 在 `main@74229cd...`，cache hydration/offline product state 在 PR #66 / `main@02e309b8...`，current-loaded-set filter/search 在 PR #67 / `main@9aaef8c...`，explorer/product hardening 在 PR #68 后闭合；PR #69 / `main@bb7a24d...` 进一步修正 RN URL compat 对 Horizon `order/limit` query 的保留。fresh 双端 aggregate 已完成并把 S13 提升为 `L3 / L4 partial`，不再有当前 Stage 4 产品语义切片；后续只保留 repeatable gate、动态字体与读屏 focus-order/tap-driven 的 L4 深度债务。其他缓存 schema/失效、Backend 边界、Mainnet/Developer Mode、最终 Application ID/iOS identity 与 release signing 继续按 Stage 8/8B/9 顺序推进，不提前宣称 L4 或发布成熟。
