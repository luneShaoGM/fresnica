...
6. 上述 S07 iOS cancel 缺口只阻止 S07 成熟度升级和最终发布签收，不再阻塞已完成 aggregate acceptance 的 S30 Trustline 或 S13 Activity。S30 已完成窄产品 hardening、独立 review/merge 与 post-#59 双端 aggregate evidence；S13 read-model integrity 与 operation-family/participants 已进入 `main@8d3aba7f...`，History cache contract 已在 PR #64 合并为 `main@b05a8757...`，cache/repository integrity 已由 PR #65 合并为 `main@74229cd...`，hydration/offline 产品状态已由 PR #66 合并为 `main@02e309b8...`。当前 current-loaded-set filter/search 代码目标为 `925cd5f7...`：增加独立 Trustlines 筛选、本地稳定字段搜索、分区切换重置、同账户详情返回/refresh 保留、显式 no-match 与手动 load-more 重应用；不引入 Horizon 全历史搜索、自动翻页、cache schema/cursor、explorer、交易或 recovery 语义。该切片必须先独立 review/merge，随后主线顺序才进入 **explorer/product hardening → 双端 aggregate → 其他 Stage 4 功能 → Stage 9 / release candidate 前真实 iPhone S07 cancel acceptance**。release candidate 前必须实机闭合 `cancel → user-cancel → pending=0 → retry → submitted`；S13 与后续写账切片继续复用 Stage 2.5 transaction recovery，不因该证据延期降低 exact-XDR / fail-closed 要求。
7. UI 最终视觉稿仍不阻塞上述功能施工；但新触及文案必须进入 locale，Accessibility label、动态字体和读屏顺序按切片记录，避免把可预见的 L4 债务继续扩大。
8. Activity/Operation Detail 的公开账本 UX cache 继续按 §4.2 与 S13 behavior spec 分层实施：repository integrity 已在 `main@74229cd...` 闭合，cache hydration/offline product state 已随 PR #66 进入 `main@02e309b8...`；当前 `925cd5f7...` 仅闭合 current-loaded-set filter/search，并继续保持 S13 为 `L3 partial`。在该独立 filter/search PR 完成 review/merge 前不得开始 explorer/product hardening；合并后才进入 explorer/product hardening，再做双端 aggregate。其他缓存 schema/失效、Backend 边界、Mainnet/Developer Mode、最终 Application ID/iOS identity 与 release signing 继续按 Stage 8/8B/9 顺序推进，不提前宣称发布成熟。
